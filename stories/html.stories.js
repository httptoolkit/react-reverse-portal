...
storiesOf("Portals", module)
  .add("render things in different places", () => {
    ...
    return (
      <div>
        <div>
          Portal defined here:
          <InPortal node={portalNode}>Hi!</InPortal>
        </div>
      ...
      return (
        <div>
          <InPortal node={portalNode}>
            <video
              src="https://media.giphy.com/media/l0HlKghz8IvrQ8TYY/giphy.mp4"
              controls
              loop
            />
          </InPortal>
  ...
  .add(
    "renders reliably, even with frequent changes and multiple portals",
    () => {
      const portalNode = createHtmlPortalNode();
  ...
  .add("Escape container constraints with a popout (standard input)", () => {
    /**
     * In this example, you'll see that when the input is focused, the popout
     * will appear outside of the constrained container.
     *
     * However, the input will not be selected.
     */
    ...
    const MyComponent = () => {
      ...
      return (
        <div
          style={{
            /* Simulate a constrained container */
            width: "100px",
            overflow: "hidden",
          }}
        >
          ...
          {!isFocused ? (
            ...
            <ComponentB portalNode={portalNode} inputRef={inputRef} />
          )}
        </div>
      );
    };
    ...
    const ComponentB = (props) => {
      const refBounds = props.inputRef.current?.getBoundingClientRect();

      return (
        <div
          style={{ position: "absolute", top: refBounds.y, left: refBounds.x }}
        >
          <OutPortal
            node={props.portalNode} // Pull in the content from this node
          />
  ...
  .add("Escape container constraints with a popout (native select)", () => {
    /**
     * Similarly to the previous example, the select drawer will open and close immediately.
     */
  ...
  .add("Escape container constraints with a popout (custom menu)", () => {
    /**
     * This example actually seems to work fine (outside of the previously mentioned issues ^),
     * so this indicates that the ref being lost might be an issue with poor re-rendering on my end.
     */
